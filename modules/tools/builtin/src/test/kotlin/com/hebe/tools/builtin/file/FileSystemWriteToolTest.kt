...
class FileSystemWriteToolTest {
    @Test
    fun `write then read roundtrip`(@TempDir tempDir: Path) {
    ...
    @Test
    fun `write with hygiene injection is rejected`(@TempDir tempDir: Path) {
    ...
    @Test
    fun `write missing path returns Err`(@TempDir tempDir: Path) {
        val tool = FileSystemWriteTool(WorkspaceFs(tempDir))
        val ctx = mockk<ToolContext>()
