...
class MacOsLaunchdService(
    ...
    private val plistFile = agentsDir.resolve("com.hebe.agent.plist")
    ...
    override fun start(): Result<Unit> = runCatching { exec("launchctl", "start", "com.hebe.agent") }

    override fun stop(): Result<Unit> = runCatching { exec("launchctl", "stop", "com.hebe.agent") }

    override fun status(): ServiceStatus {
        if (!Files.exists(plistFile)) return ServiceStatus.NotInstalled
        return try {
            val proc = ProcessBuilder("launchctl", "list", "com.hebe.agent").start()
            val out = proc.inputStream.bufferedReader().readText()
            proc.waitFor()
            if ("com.hebe.agent" in out && "\"PID\"" in out) ServiceStatus.Running else ServiceStatus.Stopped
        } catch (_: Exception) {
            ServiceStatus.NotInstalled
        }
    }
