...
suspend fun runAllChecks(
    ...
    workspaceRoot: Path,
    observer: Observer,
): List<CheckResult> = coroutineScope {
    val checks = listOf(
...
private fun checkMcp(config: HebeConfig?): CheckResult {
    ...
    if (config.mcp.client.servers.isEmpty()) {
...
fun hasAnyFailure(results: List<CheckResult>): Boolean =
    results.any { it.status == CheckStatus.Fail }