# talos — specifikace v1 (scope)

Toto je **kontrakt rozsahu (scope contract)** pro talos v1. Slouží jako referenční dokument při rozhodování, zda daná funkce patří do aktuální verze či nikoliv ("is this in or out?").
...
Zdroje pro upstream rozhodnutí: [`talos-brainstorming-responses.md`](talos-brainstorming-responses.md), [`talos-architecture.md`](talos-architecture.md), [`talos-features.md`](talos-features.md).

---

## 1. Téze v1 (definice úspěchu)

Jeden uživatel může spustit příkaz `talos run` a získá **osobního autonomního agenta**, který:
...
6. Je rozšiřitelný pomocí **PF4J pluginů stahovaných z container registry** bez nutnosti rekompilace talos.
...
Pokud může uživatel používat talos denně po dobu dvou týdnů, aniž by bylo nutné restartovat JVM, ladit proces nebo ručně upravovat databázi — verze v1 je hotová.
...
- koog zapouzdřený za fasádou `LlmProvider` / `TalosAgent`.
...
- Trait `MemoryStore` + SQLite backend (`~/.talos/talos.db`).
- FTS5 + sqlite-vec; Reciprocal Rank Fusion při `k₀ = 60`.
- Markdown workspace (`~/.talos/workspace/`) se soubory `MEMORY.md`, `IDENTITY.md`, `HEARTBEAT.md`, `daily/YYYY-MM-DD.md`.
...
- Channel `healthCheck()` dostupný přes `talos doctor`.
...
- **MCP server**: vystavení vestavěných nástrojů talos pro ostatní agenty.
...
- PF4J `PluginManager` řídící wrapper `talos` PluginManageru.
- Modul `plugin-api` vystavující `TalosPlugin` + `PluginHost` + rozhraní schopností. `koog`, `slack-bolt` (při přidání), JDBC atd. nejsou pro pluginy viditelné — pouze `api` + `plugin-api`.
- Layout pluginu: adresář nebo fat-JAR; `plugin.properties` (PF4J) + `plugin.toml` (talos manifest).
...
- Příkazy `talos plugin install <oci-ref>`, `talos plugin install <path>` (sideload), `talos plugin list` a `talos plugin remove <name>`.
- Namespacing pluginů pro skilly: `plugin:<plugin>/<skill>`.
- ABI verze fixována v manifestu (`talos_api_version`); nekompatibilní pluginy se odmítnou načíst s jasnou chybou.

### 2.9 Skilly

- Markdown balíčky ve formátu agentskills.io pod `~/.talos/skills/`.
...
- Tool receipts: Ed25519-signed, hash-chained, append-only na disku v `~/.talos/receipts/YYYY-MM.log`.
...
- Kontroly `talos doctor`: validita konfigurace, dostupnost LLM endpointu, zdraví kanálů, manifesty pluginů, sandbox (ve v1 no-op, detekce přítomnosti), přístup ke keychain.
- `talos service install` generuje a instaluje systemd unit / launchctl plist / Windows-Service definici. Daemon mód + PID soubor.
...
- Single fat JAR přes Gradle Shadow (`talos.jar`), shell wrapper `./talos`.
...
| Latence na turn | < 250 ms na straně talos (bez LLM) | OTel span budgety |
| Paměťová stopa (idle) | < 400 MB RSS | `ps` snapshot v `talos doctor` |
...
| Načtení pluginu (lokální) | < 500 ms | Benchmark v `talos plugin install --dry-run` |
...
1. `./talos onboard` provede uživatele nastavením LLM endpointu + Telegramu a vytvoří funkční `~/.talos/config.toml`.
2. `./talos run` se spustí; `./talos doctor` hlásí zelenou pro config / LLM / kanály / keychain / pluginy.
3. Z **CLI** lze vést multi-turn chat, který volá nástroje `file_system.read`, `web_search` a `http`, přičemž potvrzení (receipts) se zapisují na disk a jsou viditelná přes `./talos status --recent`.
...
7. **Plugin**: lze publikovat in-tree `hello-world` plugin do lokální OCI registry, poté provést `talos plugin install <ref>` a zavolat jeho nástroj z chatu. Stejný plugin načtený s `signature_mode = required` + platným Ed25519 podpisem funguje; bez podpisu se odmítne.
8. **MCP server**: `talos mcp serve --stdio` z Claude Desktop / Cursor umožní tomuto klientovi volat nástroj `file_system` talos.
...
12. **Potvrzení (Receipts)**: každé volání nástroje je v `~/.talos/receipts/YYYY-MM.log`, hash řetězec sedí a `talos memory show receipts/2026-04.log --verify` vrací OK.
13. **Služba**: `./talos service install --systemd` vygeneruje unit; unit spustí talos; `systemctl restart` korektně zastaví a obnoví proces (běžící turny skončí čistě, čekající schválení jsou obnovena).
14. **Estop**: `./talos estop` uprostřed volání nástroje zastaví smyčku, nezanechá žádné zombie procesy a log potvrzení zaznamená přerušení.
