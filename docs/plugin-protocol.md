...
**Classloader isolation:** plugins see only `api` and `plugin-api`. Attempting to import `com.hebe.core.*` or any other internal module fails to load. This is verified by a negative test in the CI suite.
...
- Plugins may only import classes from `com.hebe.api.*`, `com.hebe.plugin.*`, and their declared third-party dependencies (bundled in `lib/`).
...
- They must not reference `Class.forName("com.hebe.core.*")` or any other internal package. This is caught by the negative-load test.
