# talos — v1 specs (scope)

This is the **scope contract** for talos v1. It is the document we point at when deciding "is this in or out?"
...
Source of upstream decisions: [`talos-brainstorming-responses.md`](talos-brainstorming-responses.md), [`talos-architecture.md`](talos-architecture.md), [`talos-features.md`](talos-features.md).

---

## 1. v1 thesis (what success looks like)

A single human can run `talos run` and end up with **a personal autonomous agent** that:
...
6. Can be extended with **PF4J plugins pulled from a container registry** without recompiling talos.
...
If a human can use talos daily for two weeks without us having to restart it, debug it from the JVM, or hand-edit the database — v1 is done.
...
- koog wrapped behind talos's `LlmProvider` / `TalosAgent` facade.
...
- `MemoryStore` trait + SQLite backend (`~/.talos/talos.db`).
- FTS5 + sqlite-vec; Reciprocal Rank Fusion at `k₀ = 60`.
- Markdown workspace (`~/.talos/workspace/`) with `MEMORY.md`, `IDENTITY.md`, `HEARTBEAT.md`, `daily/YYYY-MM-DD.md`.
...
- Channel `healthCheck()` surfaced by `talos doctor`.
...
- **MCP server**: expose talos's built-in tools to other agents.
...
- PF4J `PluginManager` driving a `talos` PluginManager wrapper.
- `plugin-api` module exposing `TalosPlugin` + `PluginHost` + capability interfaces. `koog`, `slack-bolt` (when added), JDBC, etc. are *not* visible to plugins — only `api` + `plugin-api`.
- Plugin layout: directory or fat-JAR; `plugin.properties` (PF4J) + `plugin.toml` (talos manifest).
...
- `talos plugin install <oci-ref>` and `talos plugin install <path>` (sideload) and `talos plugin list` and `talos plugin remove <name>`.
- Plugin namespacing for skills: `plugin:<plugin>/<skill>`.
- ABI version pinned in manifest (`talos_api_version`); incompatible plugins refuse to load with a clear error.

### 2.9 Skills

- Markdown bundles in agentskills.io format under `~/.talos/skills/`.
...
- Tool receipts: Ed25519-signed, hash-chained, append-only on disk at `~/.talos/receipts/YYYY-MM.log`.
...
- `talos doctor` checks: config validity, LLM endpoint reachability, channel health, plugin manifests, sandbox (no-op in v1, presence detection), keychain access.
- `talos service install` generates and installs systemd unit / launchctl plist / Windows-Service definition. Daemon mode + PID file.
...
- Single fat JAR via Gradle Shadow (`talos.jar`), shell wrapper `./talos`.
...
| Per-turn latency overhead | < 250 ms talos-side (excludes LLM) | OTel span budgets |
| Memory footprint (idle) | < 400 MB RSS | `ps` snapshot in `talos doctor` |
...
| Plugin load (cached, local) | < 500 ms | Bench in `talos plugin install --dry-run` |
...
1. `./talos onboard` walks me through an LLM endpoint + Telegram setup and produces a working `~/.talos/config.toml`.
2. `./talos run` boots; `./talos doctor` reports green for config / LLM / channels / keychain / plugins.
3. From the **CLI**, I can have a multi-turn chat that calls `file_system.read`, `web_search`, and `http` tools, with receipts written to disk and visible via `./talos status --recent`.
...
7. **Plugin**: I can publish the in-tree `hello-world` plugin to a local OCI registry, then `talos plugin install <ref>`, then call its tool from a chat turn. Same plugin loaded with `signature_mode = required` + a valid Ed25519 signature works; loaded without a signature it refuses.
8. **MCP server**: `talos mcp serve --stdio` from Claude Desktop / Cursor lets that client call talos's `file_system` tool.
...
12. **Receipts**: every tool call is in `~/.talos/receipts/YYYY-MM.log`, the chain hash verifies, and `talos memory show receipts/2026-04.log --verify` returns OK.
13. **Service**: `./talos service install --systemd` generates a unit; the unit starts talos; `systemctl restart` cleanly stops and resumes (in-flight turns terminate cleanly, pending approvals are restored).
14. **Estop**: `./talos estop` mid-tool-call halts the loop, leaves no zombie processes, and the receipts log records the abort.
...
| M9 | **Operations** | `talos doctor`, `talos service install`, `talos onboard`, fat-JAR build via Shadow, structured logs, OTel wiring. |
...
(Pulled from `talos-brainstorming.md` §4 and updated after the responses.)
