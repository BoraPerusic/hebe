...
- **[?]** — explicitly contested / undecided — see `talos-brainstorming.md`

> **Deltas baked in (after `talos-brainstorming-responses.md`):**
> - All WASM features removed. Plugins are JVM JARs loaded via **PF4J**, distributed via OCI container registry (ACR). See `talos-architecture.md` §8.
...
| `TalosPlugin` base class (extends `org.pf4j.Plugin`) — lives in sibling `plugin-api` module | **v1** | Contract for JVM plugin JARs. Methods return `tools/channels/memoryStores/observers`. PF4J handles classloader + lifecycle. |
...
Closed in `talos-brainstorming-responses.md` §3.4 + §7: v1 ships **one OpenAI-compatible client, BYOK.** The user's LLM Gateway (which speaks the OpenAI protocol) is the upstream routing layer; talos doesn't try to duplicate that responsibility.
...
| Cost tracking per call (token counts → daily $ budget) | **v1** | Informational at the talos layer (the gateway is authoritative for billing). |
...
| Provider router (per-hint model selection) | [L] | Gateway concern. Not talos's. |
| Fallback chain on transport errors | [L] | Gateway concern. Not talos's. |
...
| Skill registry — bundled, user (`~/.talos/skills/`), plugin-bundled | **v1** | Plugin-namespaced IDs `plugin:<name>/<skill>`. |
...
Closed in `talos-brainstorming-responses.md` §6.2 (PF4J), §6.4 (signature_mode optional), §6.6 (no hot-reload), §6.7 (OCI/ACR distribution).

| Feature | Tier | Notes |
|---|---|---|
| **PF4J as the plugin framework** | **v1** | `PluginManager`, `Plugin`, `Extension` from PF4J; classloader isolation provided. |
| `plugin-api` module (depends on `api` only) hosting `TalosPlugin` + `PluginHost` | **v1** | Plugin authors compile against `plugin-api`, not the kernel. |
| Plugin `plugin.properties` (PF4J standard) + `plugin.toml` (talos-specific) | **v1** | Properties: id/version/provider/plugin.class. TOML: capabilities/permissions/signature/allowlist_domains/talos_api_version. |
...
| `talos plugin install <oci-ref>` (e.g. `acr.example.com/talos-plugins/linear:0.3.1`) | **v1** | Pull → verify signature → cache → register with PF4J. |
| `talos plugin install <local-path>` for local dev | **v1** | Sideloading from a local JAR / directory. |
| Plugin cache at `~/.talos/cache/oci/<sha256>/` | **v1** | Content-addressed; safe to delete. |
...
| ABI version pinning per plugin | **v1** | TOML `talos_api_version`; refuse incompatible. |
| Hot reload on plugin file change | [L] | Closed in §6.6: not a priority. Plugin updates → restart talos. |
...
Closed in `talos-brainstorming-responses.md` §1.3 + §7: **v1 ships Web Console + CLI + Telegram. Nothing else.**
...
| `Channel.healthCheck()` | **v1** | Surfaced in `talos doctor`. |
...
| MCP server (expose talos tools to other agents) | **v1** | stdio + SSE/WebSocket via Ktor. |
...
| Markdown workspace (`~/.talos/workspace/`) | **v1** | Memory-as-FS. `MEMORY.md`, `IDENTITY.md`, `SOUL.md`, `USER.md`, `AGENTS.md`, `HEARTBEAT.md`, `daily/YYYY-MM-DD.md`. |
...
| Feature | Tier | Notes |
|---|---|---|
| SQLite default at `~/.talos/talos.db` | **v1** | Closed in §1.8. |
...
| Tool receipts (Ed25519, hash-chained, on disk) | **v1** | Append-only `~/.talos/receipts/YYYY-MM.log`. |
...
| Emergency stop (`talos estop`) | **v1** | Halts in-flight tool calls. |
...
| `talos sop run/list/validate` | **v2** | |

## 14. Operations

| Feature | Tier | Notes |
|---|---|---|
| `talos run`, `agent`, `service`, `doctor`, `pairing`, `estop` subcommands | **v1** | |
| `talos doctor` — config / providers / channels / sandbox detect | **v1** | Self-diagnose env issues. |
| `talos service install/start/stop` (systemd/launchctl/Windows-Service) | **v1** | Don't make users write unit files. |
| Tunnel for public exposure (cloudflared, ngrok, tailscale, custom cmd) | **v2** | Pluggable; managed child process. |
| Onboarding wizard (`talos onboard`) | **v1** | Picks LLM provider, wires first channel, generates default config. |
...
| **`plugin-template/`** Gradle template (Kotlin) | **v1** | `TalosPlugin` skeleton, sample `Tool`, `plugin.toml`, `plugin.properties`, ACR-publish task. |
...
| Project skeleton generator (`talos scaffold tool foo`) | **v2** | |
...
| Architecture doc (`docs/plan/talos-architecture.md`) | **v1** | This already exists. |
...
- **Multi-tenant / SaaS.** Out of scope. One talos instance per human.
